...
namespace HMProtection.UI
{
    ...
    public sealed class SceneSelectUI : MonoBehaviour
    {
        ...
        private Card Bind(GameObject go, LevelEntry entry, int index)
        {
            ...
            for (int listener = 0; listener < button.onClick.GetPersistentEventCount(); listener++)
                button.onClick.SetPersistentListenerState(listener, UnityEngine.Events.UnityEventCallState.Off);
            button.onClick.RemoveAllListeners();
            button.onClick.AddListener(() => Select(index));
            button.interactable = available;
            if (!available) button.navigation = new UnityEngine.UI.Navigation { mode = UnityEngine.UI.Navigation.Mode.None };
        ...
        private string ResolveScenePath(LevelEntry entry)
        {
            if (entry == null) return null;
            if (DefinitionFor(entry) != null) return DefinitionFor(entry).scenePath;
            if (string.IsNullOrEmpty(entry.sceneName)) return null;
            for (int i = 0; i < SceneManager.sceneCountInBuildSettings; i++)
            {
                string path = SceneUtility.GetScenePathByBuildIndex(i);
                if (path == entry.sceneName || System.IO.Path.GetFileNameWithoutExtension(path) == entry.sceneName) return path;
            }
            return null;
        }
        ...
        private HMProtection.EntityAdapters.LevelDefinition DefinitionFor(LevelEntry entry) =>
            entry != null && officeDefinition != null && officeDefinition.levelId == entry.id ? officeDefinition : null;

        private IEnumerator LoadLevel(LevelEntry entry, string scenePath)
        {
            var definition = DefinitionFor(entry);
            if (definition != null || WantsQuizEntry(entry)) OfficeFireChoiceFlow.RequestOnNextOfficeLoad();
            loading = true;
            RefreshInteractable();
            ...
            string error;
            bool accepted = definition != null
                ? AppNavigationService.Instance.TryLoadDefinition(definition, out operation, out error)
                : AppNavigationService.Instance.TryLoadSingle(scenePath, out operation, out error);
            if (!accepted)
            {
                yield return FailLoad(withCg, "Unable to load " + title + ". Please try again.", error);
                yield break;
            }
            if (withCg) cgTransition.BeginCgTransition(operation);
            ...
            if (operation.State == NavigationState.Failed || operation.State == NavigationState.Cancelled)
            {
                ...
                yield return FailLoad(withCg, "Unable to load " + title + ". Please try again.", operation.Error);
                yield break;
            }
